...
!-----------------------------------------------------------------------------------------
   ...
   PUBLIC :: lmder_SW_VG_dist
   PUBLIC :: lmder_SW_CB_dist
   PUBLIC :: lmpar
   PUBLIC :: qrfac
   PUBLIC :: qrsolv
...
CONTAINS
   ...
   !---------------------------------
   attributes(device) SUBROUTINE normalize_longitude (lon)
   ...
   !--------------------------------------------------
   attributes(device) SUBROUTINE expand_list_int32 (list, percent)
   ...
   !--------------------------------------------------
   attributes(device) SUBROUTINE expand_list_int64 (list, percent)
   ...
   !--------------------------------------------------
   attributes(device) SUBROUTINE expand_list_real8 (list, percent)
   ...
   !--------------------------------------------------
   attributes(device) SUBROUTINE append_to_list (list1, list2)
   ...
   !--------------------------------------------------
   attributes(device) SUBROUTINE insert_into_sorted_list1_int32 (x, n, list, iloc, is_new_out)
   ...
   !--------------------------------------------------
   attributes(device) SUBROUTINE insert_into_sorted_list1_int64 (x, n, list, iloc, is_new_out)
   ...
   !--------------------------------------------------
   attributes(device) SUBROUTINE insert_into_sorted_list2 (x, y, n, xlist, ylist, iloc, is_new_out)
   ...
   integer, allocatable :: temp (:)
      ...
      IF (is_new) THEN
         IF (iloc <= n) THEN
            ...
            allocate (temp(n0))
            temp = xlist
            xlist(iloc+1:n+1) = temp(iloc:n)
            temp = ylist
            ylist(iloc+1:n+1) = temp(iloc:n)
   ...
   !-----------------------------------------------------
   attributes(device) recursive SUBROUTINE quicksort_int32 (nA, A, order)
   ...
   !-----------------------------------------------------
   attributes(device) recursive SUBROUTINE quicksort_int64 (nA, A, order)
   ...
   !-----------------------------------------------------
   attributes(device) recursive SUBROUTINE quicksort_real8 (nA, A, order)
   ...
   !-----------------------------------------------------
   attributes(device) SUBROUTINE unpack_inplace_int32 (din, msk, dout)
   ...
   !-----------------------------------------------------
   attributes(device) SUBROUTINE unpack_inplace_real8 (din, msk, dout)
   ...
   !-----------------------------------------------------
   attributes(device) SUBROUTINE unpack_inplace_lastdim_real8 (din, msk, dout)
   ...
   !---------------------------------------------------
   attributes(device) integer FUNCTION num_max_frequency (data_in)
      ...
      DO i = 1, ndata
         ...
         IF (is_new) THEN
            IF (iloc < n) THEN
               ...
               allocate (temp(n0))
               temp = cnts
               cnts(iloc+1:ndata) = temp(iloc:ndata-1)
               deallocate (temp)
            ENDIF
            cnts(iloc) = 1
         ELSE
   ...
   !----------------------------------------------------
   attributes(device) SUBROUTINE lmder_SW_VG_dist ( m, n, x, fvec, fjac, ldfjac, ftol, xtol, gtol, maxfev, &
   ...
   !
      iflag = 1
      CALL SW_VG_dist ( m, n, x, fvec, fjac, ldfjac, iflag, xdat, npoint, ydat, ydatks, nptf, phi, k_s, isiter, L_vgm)
      nfev = 1
      IF ( iflag < 0 ) THEN
         go to 300
      ENDIF
   ...
   !
         ...
         ENDIF
   !
   !     IF requested, call SW_VG_dist to enable printing of iterates.
   !
         IF ( 0 < nprint ) THEN
            iflag = 0
   ...
   !
            iflag = 1
            CALL SW_VG_dist ( m, n, wa2, wa4, fjac, ldfjac, iflag, xdat, npoint, ydat, ydatks, nptf, phi, k_s, isiter, L_vgm)
   ...
   !
      ...
      RETURN
   END SUBROUTINE lmder_SW_VG_dist

   !----------------------------------------------------
   attributes(device) SUBROUTINE lmder_SW_CB_dist ( m, n, x, fvec, fjac, ldfjac, ftol, xtol, gtol, maxfev, &
     diag, mode, factor, nprint, info, nfev, njev, ipvt, qtf, xdat, npoint, ydat, &
     ydatks, nptf, phi, k_s, isiter, L_vgm)
   ...
   !
      iflag = 1
      CALL SW_CB_dist ( m, n, x, fvec, fjac, ldfjac, iflag, xdat, npoint, ydat, ydatks, nptf, phi, k_s, isiter )
      nfev = 1
      IF ( iflag < 0 ) THEN
         go to 300
      ENDIF
   ...
   !
         ...
         ENDIF
   !
   !     IF requested, call SW_CB_dist to enable printing of iterates.
   !
         IF ( 0 < nprint ) THEN
            iflag = 0
            IF ( mod ( iter - 1, nprint ) == 0 ) THEN
   ...
   !
            iflag = 1
            CALL SW_CB_dist ( m, n, wa2, wa4, fjac, ldfjac, iflag, xdat, npoint, ydat, ydatks, nptf, phi, k_s, isiter )
   ...
   !
      ...
      RETURN
   END SUBROUTINE lmder_SW_CB_dist

   attributes(device) SUBROUTINE lmpar ( n, r, ldr, ipvt, diag, qtb, delta, par, x, sdiag )
   ...
   attributes(device) SUBROUTINE qrfac ( m, n, a, lda, pivot, ipvt, lipvt, rdiag, acnorm )
   ...
   attributes(device) SUBROUTINE qrsolv ( n, r, ldr, ipvt, diag, qtb, x, sdiag )
   ...
   attributes(device) FUNCTION enorm ( n, x )
   ...
   attributes(device) SUBROUTINE tridia (n, a, b, c, r, u)
   ...
   ! -----------------------------------------------------------------
   attributes(device) SUBROUTINE polint(xa,ya,n,x,y)
   ...
   attributes(device) SUBROUTINE SW_CB_dist ( m, n, x, fvec, fjac, ldfjac, iflag, xdat, npoint, ydatc, ydatcks, nptf, phi, k_s, isiter)
   ...
   attributes(device) SUBROUTINE SW_VG_dist ( m, n, x, fvec, fjac, ldfjac, iflag, xdat, npoint, ydatv, ydatvks, nptf, phi, k_s, isiter, L_vgm)
