...
MODULE MOD_LeafTemperature_CUDA
...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE LeafTemperature ( ipatch, &
            patchtype     ,ivt        ,deltim     ,csoilc     ,dewmx      ,htvp       ,&
...
!=======================================================================
   ...
   USE MOD_CanopyLayerProfile_CUDA
   USE MOD_TurbulenceLEddy_CUDA
   USE MOD_AssimStomataConductance_CUDA
   USE MOD_PlantHydraulic_CUDA, only:PlantHydraulicStress_twoleaf
   USE MOD_Ozone_CUDA, only: CalcOzoneStress
   USE MOD_Qsadv_CUDA
...
!-------------------------- Dummy Arguments ----------------------------
   ...
   integer,  intent(in) :: ipatch,patchtype,ivt
   real(r8), intent(in) :: &
...
! assign iteration parameters
   ...
   real(r8) :: &
      ...
      t_beta,     &! coefficient of convective velocity [-]
...
!-----------------------------------------------------------------------
      ...
      t_beta = 1.      !- (in computing W_*)
      z0mg = (1.-fsno)*zlnd + fsno*zsno
      z0hg = z0mg
      z0qg = z0mg

      z0m    = htop * d_z0mr(patchtype)
      displa = htop * d_displar(patchtype)
...
!     rsoil = 5.22 * 1.e-6
      ...
      IF (d_DEF_HEIGHT_mode == 1) THEN

         IF (hu <= htop+1) THEN
            hu_ = htop + 1.
            IF (taux == spval) & ! only print warning for the first time-step
               print *, 'Warning: the obs height of u less than htop+1, set it to htop+1.'
         ENDIF

         IF (ht <= htop+1) THEN
            ht_ = htop + 1.
            IF (taux == spval) & ! only print warning for the first time-step
               print *, 'Warning: the obs height of t less than htop+1, set it to htop+1.'
         ENDIF

         IF (hq <= htop+1) THEN
            hq_ = htop + 1.
            IF (taux == spval) & ! only print warning for the first time-step
               print *, 'Warning: the obs height of q less than htop+1, set it to htop+1.'
         ENDIF
      ...
      IF(zldis <= 0.0) THEN
         print *, 'the obs height of u less than the zero displacement heght'
         stop
      ENDIF
         ...
         IF(lai .gt. 0.001) THEN
            ...
            IF (d_DEF_USE_PLANTHYDRAULICS) THEN
               ...
               CALL PlantHydraulicStress_twoleaf (       nl_soil    ,nvegwcs    ,&
                     d_z_soi    ,d_dz_soi     ,rootfr     ,psrf       ,qsatl      ,&
                     qaf        ,tl         ,rb         ,rss        ,raw        ,&
...
! latent heat fluxes and their derivatives
         ...
         IF (.not. d_DEF_USE_PLANTHYDRAULICS) THEN
            ! IF(etr.ge.etrc)THEN
            !    etr = etrc
            !    etr_dtl = 0.
            ! ENDIF
         ELSE
            ...
            IF(abs(etr - sum(rootflux)) .gt. 1.e-7)THEN
               print *, 'Warning: water balance violation in vegetation PHS', &
                  ipatch,d_p_iam_glb, etr, sum(rootflux), abs(etr-sum(rootflux))
               stop
            ENDIF
         ENDIF
...
!-----------------------------------------------------------------------
         ...
         um = max(ur,.1)
         ELSE
         ! IF (d_DEF_USE_CBL_HEIGHT) THEN !//TODO: Shaofeng, 2023.05.18
         !    zii = max(5.*hu_,hpbl)
         ! ENDIF !//TODO: Shaofeng, 2023.05.18
         wc = (-grav*ustar*thvstar*zii/thv)**(1./3.)
         wc2 = t_beta*t_beta*(wc*wc)
         um = sqrt(ur*ur+wc2)
         ENDIF
...
! canopy fluxes and total assimilation amd respiration
      ...
      IF (d_DEF_USE_PLANTHYDRAULICS) THEN
         ...
         ELSE
            rootflux = rootflux + d_dz_soi / sum(d_dz_soi) * etr_dtl* dtl(it-1)
         ENDIF
...
!-----------------------------------------------------------------------
      ...
      IF(abs(err) .gt. .2) &
      print 'energy imbalance in LeafTemperature.F90',it-1,err,sabv,irab,fsenl,hvap*fevpl,hprl,dheatl
#endif

!-----------------------------------------------------------------------
! Update dew accumulation (kg/m2)
!-----------------------------------------------------------------------
      IF (d_DEF_Interception_scheme .eq. 1) THEN
      ...
      ELSEIF (d_DEF_Interception_scheme .eq. 2) THEN!CLM4.5
         ldew = max(0., ldew-evplwet*deltim)

      ELSEIF (d_DEF_Interception_scheme .eq. 3) THEN !CLM5
      ...
      ELSEIF (d_DEF_Interception_scheme .eq. 4) THEN !Noah-MP
         IF (ldew_rain .gt. evplwet*deltim) THEN
            ldew_rain = ldew_rain-evplwet*deltim
            ldew_snow = ldew_snow
            ldew=ldew_rain+ldew_snow
      ...
      ELSEIF (d_DEF_Interception_scheme .eq. 5) THEN !MATSIRO
         IF (ldew_rain .gt. evplwet*deltim) THEN
            ldew_rain = ldew_rain-evplwet*deltim
            ldew_snow = ldew_snow
            ldew=ldew_rain+ldew_snow
      ...
      ELSEIF (d_DEF_Interception_scheme .eq. 6) THEN !VIC
         IF (ldew_rain .gt. evplwet*deltim) THEN
            ldew_rain = ldew_rain-evplwet*deltim
            ldew_snow = ldew_snow
            ldew=ldew_rain+ldew_snow
         ELSE
            ...
            ldew      = ldew_snow
         ENDIF
      ELSEIF (d_DEF_Interception_scheme .eq. 7) THEN !JULES
            IF (ldew_rain .gt. evplwet*deltim) THEN
               ldew_rain = ldew_rain-evplwet*deltim
               ldew_snow = ldew_snow
               ldew=ldew_rain+ldew_snow
            ELSE
               ...
               ldew      = ldew_snow
            ENDIF
      ELSEIF (d_DEF_Interception_scheme .eq. 8) THEN !JULES
            IF (ldew_rain .gt. evplwet*deltim) THEN
               ldew_rain = ldew_rain-evplwet*deltim
               ldew_snow = ldew_snow
               ldew=ldew_rain+ldew_snow
...
!----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE dewfraction (sigf,lai,sai,dewmx,ldew,ldew_rain,ldew_snow,fwet,fdry)
   !DESCRIPTION
   !===========
...
END MODULE MOD_LeafTemperature_CUDA
