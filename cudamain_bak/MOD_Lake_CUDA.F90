...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE newsnow_lake ( USE_Dynamic_Lake, &
            ! "in" arguments
            ! ---------------
...
!-----------------------------------------------------------------------
      ...
      ! if (snowdp < 2.2e-16) snowdp = 0.

   END SUBROUTINE newsnow_lake



   attributes(device) SUBROUTINE laketem (&
           ! "in" arguments
           ! -------------------
...
! -----------------------------------------------------------------
   ...
   USE MOD_Qsadv_CUDA
   USE MOD_SoilThermalParameters_CUDA
   USE MOD_Utils_CUDA
...
! ======================================================================
      ...
      IF (.not. d_DEF_USE_SNICAR .or. present(urban_call)) THEN
         IF (snl == 0) THEN
...
!The actual heat flux from the ground interface into the lake, not including the light that penetrates the surface.
      ...
      IF (d_DEF_USE_SNICAR .and. .not. present(urban_call)) THEN
...
!------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE snowwater_lake ( USE_Dynamic_Lake, &
             ! "in" arguments
             ! ---------------------------
...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE roughness_lake (snl,t_grnd,t_lake,lake_icefrac,forc_psrf,&
                             cur,ustar,z0mg,z0hg,z0qg)
...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE hConductivity_lake(nl_lake,snl,t_grnd,&
                                z_lake,t_lake,lake_icefrac,rhow,&
                                dlat,ustar,z0mg,lakedepth,depthcrit,tk_lake, savedtke1)
...
! -------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE adjust_lake_layer (nl_lake, dz_lake, t_lake, lake_icefrac)
   ...
   real(r8) :: dzlak(10) = (/0.1, 1., 2., 3., 4., 5., 7., 7., 10.45, 10.45/)  ! m
      ...
      ELSE
         print *, 'Warning: lake depth is over 2000 meters!'
      ENDIF
