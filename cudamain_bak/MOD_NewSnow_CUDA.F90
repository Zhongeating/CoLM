...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE newsnow (patchtype,maxsnl,deltim,t_grnd,pg_rain,pg_snow,bifall,&
...
!-----------------------------------------------------------------------
      ...
      IF(patchtype==2 .and. t_grnd>tfrz)THEN  ! snowfall on warmer wetland
         IF (present(wetwat) .and. d_DEF_USE_VariablySaturatedFlow) THEN
            wetwat = wetwat + scv
         ENDIF
