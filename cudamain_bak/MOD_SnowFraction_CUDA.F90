...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE snowfraction (lai,sai,z0m,zlnd,scv,snowdp,wt,sigf,fsno)
...
! Fraction of soil covered by snow
      fsno = 0.0
      IF(snowdp > 1e-6 .and. scv > 1e-6) THEN
...
#if (defined LULC_IGBP_PFT || defined LULC_IGBP_PC)
   attributes(device) SUBROUTINE snowfraction_pftwrap (ipatch,zlnd,scv,snowdp,wt,sigf,fsno)
...
END MODULE MOD_SnowFraction_CUDA
! ---------- EOP ------------
