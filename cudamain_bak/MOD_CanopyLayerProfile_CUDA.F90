...
MODULE MOD_CanopyLayerProfile_CUDA
...
CONTAINS
   ...
   attributes(device) real(r8) FUNCTION uprofile(utop, fc, bee, alpha, z0mg, htop, hbot, z)

   USE MOD_Precision
   USE MOD_FrictionVelocity_CUDA
   IMPLICIT NONE
   ...
   attributes(device) real(r8) FUNCTION kprofile(ktop, fc, bee, alpha, &
   ...
   attributes(device) real(r8) FUNCTION uintegral(utop, fc, bee, alpha, z0mg, htop, hbot)
   ...
   attributes(device) real(r8) FUNCTION uintegralz(utop, fc, bee, alpha, z0mg, &
                    htop, hbot, ztop, zbot)
   ...
   attributes(device) real(r8) FUNCTION ueffect(utop, htop, hbot, &
                             z0mg, alpha, bee, fc)
   USE MOD_Precision
   IMPLICIT NONE
   ...
   attributes(device) real(r8) FUNCTION ueffectz(utop, htop, hbot, &
   ...
   attributes(device) real(r8) FUNCTION fuint(utop, ztop, zbot, &
         htop, hbot, z0mg, alpha, bee, fc)
   ...
   ! attributes(device) RECURSIVE SUBROUTINE ufindroots(ztop,zbot,zmid, &
   !    utop, htop, hbot, z0mg, alpha, roots, rootn)
   ...
   attributes(device) SUBROUTINE ufindroots(ztop, zbot, zmid, &
         utop, htop, hbot, z0mg, alpha, roots, rootn)
      ...
      ! Stack variables to simulate recursion
      real(r8) :: stack_ztop(20), stack_zbot(20), stack_zmid(20)
      integer :: stack_flag(20)
      integer :: stack_ptr
      integer, parameter :: max_stack_size = 20

      ! Local variables
      real(r8) :: udiff_ub, udiff_lb
      real(r8) :: zup, zdown, ztemp
      integer :: zflag

      ! Initialize the stack with the initial interval
      stack_ptr = 1
      stack_ztop(stack_ptr) = ztop
      stack_zbot(stack_ptr) = zbot
      stack_zmid(stack_ptr) = zmid
      stack_flag(stack_ptr) = 1

      ! Process the stack iteratively
      DO WHILE (stack_ptr > 0 .and. rootn < 3)
         zflag = stack_flag(stack_ptr)
         ! Pop the top of the stack
         IF (zflag == 1) THEN
            zup = stack_ztop(stack_ptr)
            zdown = stack_zmid(stack_ptr)
            ztemp = zdown
            stack_flag(stack_ptr) = 0
         ELSE
            zup = stack_zmid(stack_ptr)
            zdown = stack_zbot(stack_ptr)
            ztemp = zup
            stack_ptr = stack_ptr - 1
         ENDIF

         udiff_ub = udiff(zup, utop, htop, hbot, z0mg, alpha)
         udiff_lb = udiff(zdown, utop, htop, hbot, z0mg, alpha)
         ...
         IF (udiff_ub * udiff_lb == 0) THEN
            ...
            ENDIF
         ELSE IF (udiff_ub * udiff_lb < 0) THEN
            IF (zup - zdown < 0.01) THEN
               rootn = rootn + 1 ! Root found
               IF (rootn > 2) THEN
                  print *, "Warning: U root number > 2, attention!"
                  RETURN
               ENDIF
               roots(rootn) = (zup + zdown) / 2.
            ELSE
               ! Push the new interval [ztop, zmid] onto the stack
               stack_ptr = stack_ptr + 1
               IF (stack_ptr > max_stack_size) THEN
                  print *, "Stack overflow, abort!"
                  RETURN
               ENDIF
               stack_ztop(stack_ptr) = zup
               stack_zbot(stack_ptr) = zdown
               stack_zmid(stack_ptr) = (zup + zdown) / 2.
               stack_flag(stack_ptr) = 1
            ENDIF
         ENDIF
      ENDDO
   ...
   attributes(device) real(r8) FUNCTION udiff(z, utop, htop, hbot, z0mg, alpha)
   ...
   real(r8) :: uexp, ulog
      ...
      udiff = uexp - ulog
      IF (abs(udiff) < 1e-15) udiff = 0._r8 ! to avoid numerical error

      RETURN
   END FUNCTION udiff


   attributes(device) real(r8) FUNCTION kintegral(ktop, fc, bee, alpha, z0mg, &
   ...
   attributes(device) real(r8) FUNCTION frd(ktop, htop, hbot, &
   ...
   attributes(device) real(r8) FUNCTION fkint(ktop, ztop, zbot, htop, hbot, &
         z0h, obu, ustar, fac, alpha, bee, fc)
   ...
   ! attributes(device) RECURSIVE SUBROUTINE kfindroots(ztop,zbot,zmid, &
   !    ktop, htop, hbot, obu, ustar, fac, alpha, roots, rootn)
   ...
   attributes(device) SUBROUTINE kfindroots(ztop, zbot, zmid, &
         ktop, htop, hbot, obu, ustar, fac, alpha, roots, rootn)
      ...
      DO WHILE (stack_ptr > 0 .and. rootn < 3)
         ...
         kdiff_ub = kdiff(zup, ktop, htop, hbot, obu, ustar, fac, alpha)
         kdiff_lb = kdiff(zdown, ktop, htop, hbot, obu, ustar, fac, alpha)
   ...
   attributes(device) real(r8) FUNCTION kdiff(z, ktop, htop, hbot, &
         obu, ustar, fac, alpha)
   ...
   attributes(device) SUBROUTINE cal_z0_displa (lai, h, fc, z0, displa)
...
END MODULE MOD_CanopyLayerProfile_CUDA
