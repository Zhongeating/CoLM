...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE meltf (patchtype,is_dry_lake,lb,nl_soil,deltim, &
...
!-----------------------------------------------------------------------
   ...
   USE MOD_Hydro_SoilFunction_CUDA
   IMPLICIT NONE
...
! supercooling water
      IF (d_DEF_USE_SUPERCOOL_WATER) THEN
         DO j = 1, nl_soil
            supercool(j) = 0.0
            IF(t_soisno(j) < tfrz .and. ((patchtype <=2) .or. (is_dry_lake == 1))) THEN
      ...
      DO j = lb, nl_soil
         IF(imelt(j) > 0)THEN
            ...
            IF(j > lb)THEN             ! => not the top layer
               IF (j==1 .and. d_DEF_SPLIT_SOILSNOW .and. ((patchtype<3) .or. (is_dry_lake == 1))) THEN
               ...
               ENDIF
            ELSE                       ! => top layer
               IF (j==1 .or. (.not.d_DEF_SPLIT_SOILSNOW) .or. patchtype==3) THEN
                                       ! -> soil layer
...
! The rate of melting and freezing
   ...
   attributes(device) SUBROUTINE meltf_snicar (patchtype,is_dry_lake,lb,nl_soil,deltim, &
...
! The rate of melting and freezing
   ...
   attributes(device) SUBROUTINE meltf_urban (lb,nl_soil,deltim, &
                   fact,brr,hs,dhsdT, &
                   t_soisno_bef,t_soisno,wliq_soisno,wice_soisno,imelt, &
                   scv,snowdp,sm,xmf)
...
END MODULE MOD_PhaseChange_CUDA
! ---------- EOP ------------
