...
MODULE MOD_AssimStomataConductance_CUDA
...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE stomata (vmax25,effcon,slti,hlti,shti, &
...
!     assimilation rate. ! pay attention on this iteration
      ...
      ITERATION_LOOP: DO ic = 1, iterationtotal
         ...
         IF(.not. d_DEF_USE_WUEST .or. abs(c4 - 1) .lt. 0.001)THEN
...
!-----------------------------------------------------------------------
         ...
         IF(d_DEF_USE_WUEST .and. .not. abs(c4 - 1) .lt. 0.001)THEN
            IF(omc .lt. ome)THEN
               pco2i = pco2i_c
            ELSE
               pco2i = pco2i_e
...
! convert gsh2o (mol m-2 s-1) to resistance rst ( s m-1)
   ...
   attributes(device) SUBROUTINE sortin( eyy, pco2y, range, gammas, ic, iterationtotal )
...
200   continue
   ...
   attributes(device) SUBROUTINE calc_photo_params(tlef, po2m, par , psrf, rstfac, rb, effcon, vmax25, &
                               trop, slti, hlti, shti, hhti, trda, trdm, cint, &
                               vm, epar, respc, omss, gbh2o, gammas, rrkk, c3, c4)
...
!     gbh2o  = gbh2o * cint(3)
   ...
   attributes(device) SUBROUTINE update_photosyn(tlef, po2m, pco2m, pco2a, par, psrf, rstfac, rb, gsh2o, &
...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE WUE_solver(gammas, lambda, co2a, ei, ea, psrf, pco2i_c, pco2i_e)
...
END MODULE MOD_AssimStomataConductance_CUDA
! -------------- EOP ---------------
