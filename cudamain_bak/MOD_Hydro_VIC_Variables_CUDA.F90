MODULE MOD_Hydro_VIC_Variables_CUDA
   USE cudafor
...
CONTAINS
   ...
   attributes(device) SUBROUTINE vic_para(porsl, theta_r, hksati, bsw, wice_soisno, wliq_soisno, fevpg, rootflux, &
                        b_infilt, Dsmax, Ds, Ws, c, &
                        soil_con, cell, Nfrost)
      ...
      integer,  intent(inout) :: Nfrost           !/**< Number of frost subareas in model */
      ...
      dltime = d_DEF_simulation_time%timestep

      CALL CoLM2VIC(d_dz_soi, soil_tmp)
   ...
   attributes(device) SUBROUTINE VIC_IceLay(lb, lp, colm_ice, vic_ice)
   ...
   attributes(device) SUBROUTINE CoLM2VIC(colm_water, vic_water)
   ...
   attributes(device) SUBROUTINE CoLM2VIC_weight(colm_water, vic_water)
   ...
   !-----------------------END Variable List-------------------------------
      ...
      DO i_vic = 1, Nlayer
         ...
         IF (i_vic == 1) THEN
            DO i_colm = 1, colm2vic_lay(i_vic)
               vic_water(i_vic) = vic_water(i_vic) + colm_water(i_colm)*d_dz_soi(i_colm)
            ENDDO
            vic_water(i_vic) = vic_water(i_vic)/sum(d_dz_soi(1:colm2vic_lay(i_vic)))
         ELSE
            DO i_colm = colm2vic_lay(i_vic-1)+1, colm2vic_lay(i_vic)
               vic_water(i_vic) = vic_water(i_vic) + colm_water(i_colm)*d_dz_soi(i_colm)
            ENDDO
            vic_water(i_vic) = vic_water(i_vic)/sum(d_dz_soi(colm2vic_lay(i_vic-1)+1:colm2vic_lay(i_vic)))
         ENDIF
      ENDDO
   ...
   attributes(device) SUBROUTINE VIC2CoLM(colm_water, vic_water)
   ...
   !-----------------------END Variable List-------------------------------
      ...
      DO i_vic = 1, Nlayer
         IF (i_vic == 1) THEN
            DO i_colm = 1, colm2vic_lay(i_vic)
               colm_water(i_colm) = vic_water(i_vic)*(d_dz_soi(i_colm)/sum(d_dz_soi(1:colm2vic_lay(i_vic))))
            ENDDO
         ELSE
            DO i_colm = colm2vic_lay(i_vic-1)+1, colm2vic_lay(i_vic)
               colm_water(i_colm) = vic_water(i_vic)*(d_dz_soi(i_colm)/sum(d_dz_soi(colm2vic_lay(i_vic-1)+1:colm2vic_lay(i_vic))))
            ENDDO
         ENDIF
      ENDDO
...
END MODULE MOD_Hydro_VIC_Variables_CUDA
