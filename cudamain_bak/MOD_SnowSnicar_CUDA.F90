...
! !USES:
   USE cudafor
   USE MOD_ConstVars_CUDA, only: maxsnl, tfrz
...
!-----------------------------------------------------------------------
   ...
   real(r8), allocatable, managed :: ss_alb_snw_drc     (:,:) ! (idx_Mie_snw_mx,numrad_snw);
   real(r8), allocatable, managed :: asm_prm_snw_drc    (:,:) ! (idx_Mie_snw_mx,numrad_snw);
   real(r8), allocatable, managed :: ext_cff_mss_snw_drc(:,:) ! (idx_Mie_snw_mx,numrad_snw);

   ! diffuse radiation weighted ice optical properties
   real(r8), allocatable, managed :: ss_alb_snw_dfs     (:,:) ! (idx_Mie_snw_mx,numrad_snw);
   real(r8), allocatable, managed :: asm_prm_snw_dfs    (:,:) ! (idx_Mie_snw_mx,numrad_snw);
   real(r8), allocatable, managed :: ext_cff_mss_snw_dfs(:,:) ! (idx_Mie_snw_mx,numrad_snw);

   ! direct & diffuse flux
   real(r8), allocatable, managed :: flx_wgt_dir (:,:,:) ! (6, 90, numrad_snw) ! direct flux, six atmospheric types, 0-89 SZA
   real(r8), allocatable, managed :: flx_wgt_dif (:,:)   ! (6, numrad_snw)     ! diffuse flux, six atmospheric types
   ...
   real(r8), allocatable, managed :: ss_alb_bc1     (:,:)  ! (numrad_snw,idx_bc_nclrds_max);
   real(r8), allocatable, managed :: asm_prm_bc1    (:,:)  ! (numrad_snw,idx_bc_nclrds_max);
   real(r8), allocatable, managed :: ext_cff_mss_bc1(:,:)  ! (numrad_snw,idx_bc_nclrds_max);

   ! external BC
   real(r8), allocatable, managed :: ss_alb_bc2     (:,:)  ! (numrad_snw,idx_bc_nclrds_max);
   real(r8), allocatable, managed :: asm_prm_bc2    (:,:)  ! (numrad_snw,idx_bc_nclrds_max);
   real(r8), allocatable, managed :: ext_cff_mss_bc2(:,:)  ! (numrad_snw,idx_bc_nclrds_max);
   !mgf--
#else
   ! hydrophiliic BC
   real(r8), allocatable, managed :: ss_alb_bc1     (:) ! (numrad_snw);
   real(r8), allocatable, managed :: asm_prm_bc1    (:) ! (numrad_snw);
   real(r8), allocatable, managed :: ext_cff_mss_bc1(:) ! (numrad_snw);

   ! hydrophobic BC
   real(r8), allocatable, managed :: ss_alb_bc2     (:) ! (numrad_snw);
   real(r8), allocatable, managed :: asm_prm_bc2    (:) ! (numrad_snw);
   real(r8), allocatable, managed :: ext_cff_mss_bc2(:) ! (numrad_snw);
#endif

   ! hydrophobic OC
   real(r8), allocatable, managed :: ss_alb_oc1     (:) ! (numrad_snw);
   real(r8), allocatable, managed :: asm_prm_oc1    (:) ! (numrad_snw);
   real(r8), allocatable, managed :: ext_cff_mss_oc1(:) ! (numrad_snw);

   ! hydrophilic OC
   real(r8), allocatable, managed :: ss_alb_oc2     (:) ! (numrad_snw);
   real(r8), allocatable, managed :: asm_prm_oc2    (:) ! (numrad_snw);
   real(r8), allocatable, managed :: ext_cff_mss_oc2(:) ! (numrad_snw);

   ! dust species 1:
   real(r8), allocatable, managed :: ss_alb_dst1     (:) ! (numrad_snw);
   real(r8), allocatable, managed :: asm_prm_dst1    (:) ! (numrad_snw);
   real(r8), allocatable, managed :: ext_cff_mss_dst1(:) ! (numrad_snw);

   ! dust species 2:
   real(r8), allocatable, managed :: ss_alb_dst2     (:) ! (numrad_snw);
   real(r8), allocatable, managed :: asm_prm_dst2    (:) ! (numrad_snw);
   real(r8), allocatable, managed :: ext_cff_mss_dst2(:) ! (numrad_snw);

   ! dust species 3:
   real(r8), allocatable, managed :: ss_alb_dst3     (:) ! (numrad_snw);
   real(r8), allocatable, managed :: asm_prm_dst3    (:) ! (numrad_snw);
   real(r8), allocatable, managed :: ext_cff_mss_dst3(:) ! (numrad_snw);

   ! dust species 4:
   real(r8), allocatable, managed :: ss_alb_dst4     (:) ! (numrad_snw);
   real(r8), allocatable, managed :: asm_prm_dst4    (:) ! (numrad_snw);
   real(r8), allocatable, managed :: ext_cff_mss_dst4(:) ! (numrad_snw);

#ifdef MODAL_AER
   ! Absorption enhancement factors for within-ice BC
   real(r8), allocatable, managed :: bcenh (:,:,:) ! (numrad_snw,idx_bc_nclrds_max,idx_bcint_icerds_max);
#endif
   ...
   ! (arrays declared here, but are set in iniTimeConst)
   !
   real(r8), allocatable, managed :: snowage_tau   (:,:,:) ! (idx_rhos_max,idx_Tgrd_max,idx_T_max)  ! snow aging parameter retrieved from lookup table [hour]
   real(r8), allocatable, managed :: snowage_kappa (:,:,:) ! (idx_rhos_max,idx_Tgrd_max,idx_T_max)  ! snow aging parameter retrieved from lookup table [unitless]
   real(r8), allocatable, managed :: snowage_drdt0 (:,:,:) ! (idx_rhos_max,idx_Tgrd_max,idx_T_max)  ! snow aging parameter retrieved from lookup table [um hr-1]
...
CONTAINS
   ...
   attributes(device) SUBROUTINE SNICAR_RT (flg_snw_ice, flg_slr_in, &
...
! #endif
   ...
   attributes(device) SUBROUTINE SNICAR_AD_RT (flg_snw_ice, flg_slr_in, &
                            coszen, snl, h2osno, frac_sno, &
                            h2osno_liq, h2osno_ice, snw_rds, &
                            mss_cnc_aer_in, albsfc, albout, flx_abs)
!-----------------------------------------------------------------------
...
! !ARGUMENTS:
  ...
  !       )
   ...
   attributes(device) SUBROUTINE SnowAge_grain( dtime          , snl            , dz              ,&
...
!    I am aware.
 ...
 !       )
   ...
   attributes(device) real(r8) FUNCTION FreshSnowRadius (forc_t)
!-----------------------------------------------------------------------
! !DESCRIPTION:
...
!-----------------------------------------------------------------------
! !USES:
   USE MOD_Aerosol_CUDA, only: fresh_snw_rds_max
...
END MODULE MOD_SnowSnicar_CUDA
! ---------- EOP ------------
