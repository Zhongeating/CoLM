...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE albland (ipatch, patchtype, deltim,&
...
!=======================================================================
   ...
   USE MOD_Precision
   USE MOD_Aerosol_CUDA, only: AerosolMasses
   USE MOD_SnowSnicar_CUDA, only: SnowAge_grain
...
!-------------------------- Local Variables ----------------------------
   ...
   real(r8) ssno_lyr_1_1  ( maxsnl+1:1 )
   real(r8) ssno_lyr_2_1  ( maxsnl+1:1 )
   real(r8) ssno_lyr_1_2  ( maxsnl+1:1 )
   real(r8) ssno_lyr_2_2  ( maxsnl+1:1 )
      ...
      IF (scv > 0.) THEN
         ...
         ELSE
            ...
            CALL SnowAlbedo(     use_snicar_frc ,use_snicar_ad  ,coszen         ,&
                 ...
                 albsno_dst(:,1),albsno_dst(:,2),ssno_lyr_1_1(:),ssno_lyr_2_1(:),&
                 ssno_lyr_1_2(:),ssno_lyr_2_2(:))

            ssno_lyr(1,1,:) = ssno_lyr_1_1(:)
            ssno_lyr(2,1,:) = ssno_lyr_2_1(:)
            ssno_lyr(1,2,:) = ssno_lyr_1_2(:)
            ssno_lyr(2,2,:) = ssno_lyr_2_2(:)
...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE twostream ( chil, rho, tau, green, lai, sai, fwet_snow, &
...
#if (defined LULC_IGBP_PFT || defined LULC_IGBP_PC)
   attributes(device) SUBROUTINE twostream_mod ( chil, rho, tau, green, lai, sai, fwet_snow, &
              coszen, albg, albv, tran, thermk, extkb, extkd, ssun, ssha )
...
#if (defined LULC_IGBP_PFT || defined LULC_IGBP_PC)
   attributes(device) SUBROUTINE twostream_wrap ( ipatch, coszen, albg, &
              albv, tran, ssun, ssha )
...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE snowage ( deltim,tg,scv,scvold,sag )
...
!
   ...
   attributes(device) SUBROUTINE SnowAlbedo( use_snicar_frc,use_snicar_ad ,coszen_col    ,&
   ...
   !-----------------------------------------------------------------------
   ! !USES:
   USE MOD_SnowSnicar_CUDA, only: SNICAR_RT, SNICAR_AD_RT
...
! temporary setting
   ...
   attributes(device) SUBROUTINE albocean (oro, scv, coszrs, alb)
