...
MODULE MOD_SoilSnowHydrology_CUDA
...
!-----------------------------------------------------------------------
   ...
   USE MOD_Precision
   USE MOD_Runoff_CUDA
   USE MOD_Hydro_VIC_CUDA
   USE MOD_Hydro_VIC_Variables_CUDA
...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE WATER_2014 (ipatch,patchtype,lb       ,nl_soil     ,deltim      ,&
...
! SNICAR model variables
              ...
              vic_b_infilt,vic_Dsmax   ,vic_Ds      ,vic_Ws                   ,&
              vic_c       ,fevpg                                               )
...
! END SNICAR model variables
   ...
   real(r8), intent(in)    :: vic_b_infilt, vic_Dsmax, vic_Ds, vic_Ws, vic_c
...
IF ((.not.d_DEF_SPLIT_SOILSNOW) .or. (patchtype==1 .and. d_DEF_URBAN_RUN)) THEN

      IF (lb>=1)THEN
         gwat = pg_rain + sm - qseva
      ELSE
         IF ((.not.d_DEF_USE_SNICAR) .or. (patchtype==1 .and. d_DEF_URBAN_RUN)) THEN
...
IF(patchtype<=1)THEN   ! soil ground only
      ...
      ELSEIF (d_DEF_Runoff_SCHEME  == 1) THEN
         ! 1: runoff scheme from VIC model

         wliq_soisno_tmp(:) = 0
         CALL Runoff_VIC(deltim, porsl, theta_r, hksati, bsw, &
                         wice_soisno(1:nl_soil), wliq_soisno(1:nl_soil), fevpg, rootflux, gwat, &
                         vic_b_infilt, vic_Dsmax, vic_Ds, vic_Ws, vic_c,&
                         rsur, rsubst, wliq_soisno_tmp(1:nl_soil))

      ELSEIF (d_DEF_Runoff_SCHEME  == 2) THEN
      ...
      ELSEIF (d_DEF_Runoff_SCHEME  == 3) THEN
      ...
      IF (LWINFILT) THEN
         ...
         IF ((flddepth .gt. 1.e-6).and.(fldfrc .gt. 0.05) .and. (patchtype == 0) ) THEN
            ...
            ELSEIF (d_DEF_Runoff_SCHEME  == 1) THEN
               wliq_soisno_tmp(:) = 0
               CALL Runoff_VIC(deltim, porsl, theta_r, hksati, bsw, &
                               wice_soisno(1:nl_soil), wliq_soisno(1:nl_soil), fevpg, rootflux, gfld, &
                               vic_b_infilt, vic_Dsmax, vic_Ds, vic_Ws, vic_c,&
...
!-----------------------------------------------------------------------
!    attributes(device) SUBROUTINE WATER_VSF (ipatch, patchtype,is_dry_lake, lb, nl_soil, deltim   ,&
...
!          IF (wetwat > d_wetwatmax) THEN
!             wdsrf  = wetwat - d_wetwatmax
!             wetwat = d_wetwatmax
!             wa     = 0.
...
!    END SUBROUTINE WATER_VSF
   ...
   attributes(device) SUBROUTINE snowwater (lb,deltim,ssi,wimp, &
                        pg_rain,qseva,qsdew,qsubl,qfros, &
...
!-----------------------------------------------------------------------
   attributes(device) SUBROUTINE SnowWater_snicar (lb,deltim,ssi,wimp, &
                        pg_rain,qseva,qsdew,qsubl,qfros, &
                        dz_soisno,wice_soisno,wliq_soisno,qout_snowb, &
...
! Aerosol Fluxes (Jan. 7, 2023)
   ...
   attributes(device) SUBROUTINE soilwater(patchtype,nl_soil,deltim,wimp,smpmin,&
...
!-----------------------------------------------------------------------
      ...
      DO j = 1, nl_soil
         IF(d_DEF_USE_PLANTHYDRAULICS .and. (patchtype/=1 .or. (.not.d_DEF_URBAN_RUN)))THEN
            IF(t_soisno(j)>=tfrz) THEN
...
! Method II:
  ...
  ! The mass balance error (mm) for this time step is
   ...
   attributes(device) SUBROUTINE groundwater (nl_soil,deltim,pondmx,&
...
END MODULE MOD_SoilSnowHydrology_CUDA
! ---------- EOP ------------
