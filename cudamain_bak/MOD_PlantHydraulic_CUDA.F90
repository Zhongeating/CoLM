...
MODULE MOD_PlantHydraulic_CUDA
...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE PlantHydraulicStress_twoleaf (nl_soil    ,nvegwcs    ,z_soi      ,&
...
!=======================================================================
   ...
   attributes(device) SUBROUTINE calcstress_twoleaf(x,nvegwcs,rstfacsun, rstfacsha, etrsun, etrsha, rootflux,&
              gb_mol, gs0sun, gs0sha, qsatl, qaf, qg, qm,rhoair,&
   ...
   !------------------------------------------------------------------------------
      ...
      IF(qflx_sun .gt. 0 .or. qflx_sha .gt. 0)THEN
         CALL getrootqflx_x2qe(nl_soil,smp,x_root_top ,z_soi,k_soil_root,k_ax_root,qeroot,dqeroot, &
                                 amx_hr(1:nl_soil-1),bmx_hr(1:nl_soil-1),cmx_hr(1:nl_soil-1),rmx_hr(1:nl_soil-1),drmx_hr,hx(1:nl_soil-1),hdx,xroot,zmm,qeroot_nl)
      ...
      ! .. when gs=0 -> B= plc( x )
         ...
         CALL getrootqflx_qe2x(nl_soil,smp,z_soi,k_soil_root,k_ax_root,qeroot,xroot,x_root_top, &
                                 amx_hr,bmx_hr,cmx_hr,rmx_hr,hx,zmm,qeroot_nl)
         x(root) = x_root_top
         DO j = 1,nl_soil
   ...
   !------------------------------------------------------------------------------
   attributes(device) SUBROUTINE spacAF_twoleaf(x,nvegwcs,dx,nl_soil,qflx_sun,qflx_sha,laisun,laisha,sai,htop,&
                   qeroot,dqeroot,kmax_sun,kmax_sha,kmax_xyl,kmax_root,&
                   psi50_sun,psi50_sha,psi50_xyl,psi50_root,ck)
!-----------------------------------------------------------------------
! !DESCRIPTION
...
!    attributes(device) SUBROUTINE getvegwp_twoleaf(x, nvegwcs, nl_soil, z_soi, gb_mol, gs_mol_sun, gs_mol_sha, &
...
!    END SUBROUTINE getvegwp_twoleaf
   ...
   attributes(device) SUBROUTINE getqflx_gs2qflx_twoleaf(gb_mol,gs_mol_sun,gs_mol_sha,qflx_sun,qflx_sha,qsatl,qaf,&
                             rhoair,psrf,laisun,laisha,sai,fwet,tl,rss,raw,rd,qg,qm,rstfacsun,rstfacsha)
!-----------------------------------------------------------------------
! !DESCRIPTION:
...
!      ENDIF
   ...
   attributes(device) SUBROUTINE getqflx_qflx2gs_twoleaf(gb_mol,gs_mol_sun,gs_mol_sha,qflx_sun,qflx_sha,qsatl,qaf, &
                      rhoair,psrf,laisun,laisha,sai,fwet,tl,rss,raw,rd,qg,qm)
!-----------------------------------------------------------------------
   ! !DESCRIPTION:
   !  calculate sunlit and shaded transpiration using gb_MOL and gs_MOL
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE getrootqflx_x2qe(nl_soil,smp,x_root_top,z_soisno,krad,kax,qeroot,dqeroot, &
                                                   amx_hr,bmx_hr,cmx_hr,rmx_hr,drmx_hr,x,dx,xroot,zmm,qeroot_nl)
...
! Local variables
   real(r8), intent(inout) :: amx_hr(nl_soil-1)  ! "a" left off diagonal of tridiagonal matrix
   real(r8), intent(inout) :: bmx_hr(nl_soil-1)  ! "b" diagonal column for tridiagonal matrix
   real(r8), intent(inout) :: cmx_hr(nl_soil-1)  ! "c" right off diagonal tridiagonal matrix
   real(r8), intent(inout) :: rmx_hr(nl_soil-1)  ! "r" forcing term of tridiagonal matrix
   real(r8), intent(inout) :: drmx_hr(nl_soil-1) ! "dr" forcing term of tridiagonal matrix for d/dxroot(1)
   real(r8), intent(inout) :: x(nl_soil-1)       ! root water potential from layer 2 to nl_soil
   real(r8), intent(inout) :: dx(nl_soil-1)      ! derivate of root water potential from layer 2 to nl_soil (dxroot(:)/dxroot(1))
   real(r8), intent(inout) :: xroot(nl_soil)     ! root water potential from layer 2 to nl_soil
   real(r8), intent(inout) :: zmm(1:nl_soil)     ! layer depth [mm]
   real(r8), intent(inout) :: qeroot_nl(1:nl_soil) ! root water potential from layer 2 to nl_soil
   ...
   attributes(device) SUBROUTINE getrootqflx_qe2x(nl_soil,smp,z_soisno,krad,kax,qeroot,xroot,x_root_top, &
                                                   amx_hr,bmx_hr,cmx_hr,rmx_hr,x,zmm,qeroot_nl)
...
! Local variables
   ...
   real(r8), intent(inout) ::  cmx_hr(nl_soil)      ! "c" right off diagonal tridiagonal matrix
   real(r8), intent(inout) ::  rmx_hr(nl_soil)      ! "r" forcing term of tridiagonal matrix
   real(r8), intent(inout) ::  x(nl_soil)           ! root water potential from layer 2 to nl_soil
   real(r8), intent(inout) ::  zmm(1:nl_soil)       ! layer depth [mm]
   real(r8), intent(inout) ::  qeroot_nl(1:nl_soil) ! root water potential from layer 2 to nl_soil
   ...
   attributes(device) FUNCTION plc(x,psi50,ck)
!-----------------------------------------------------------------------
! !DESCRIPTION
...
!    END select
   ...
   attributes(device) FUNCTION d1plc(x,psi50,ck)
!-----------------------------------------------------------------------
! !DESCRIPTION
...
END MODULE MOD_PlantHydraulic_CUDA
! ---------- EOP ------------
