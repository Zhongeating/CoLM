...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE netsolar (ipatch,deltim,dlon,patchtype,&
...
!-----------------------------------------------------------------------
      ...
      radpsec = pi/12./3600.
      IF ( d_isgreenwich ) THEN
         local_secs = d_idate(3) + nint((dlon/radpsec)/deltim)*deltim
         local_secs = mod(local_secs,86400)
      ELSE
         local_secs = d_idate(3)
      ENDIF
