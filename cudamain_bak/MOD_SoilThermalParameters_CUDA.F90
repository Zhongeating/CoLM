...
MODULE MOD_SoilThermalParameters_CUDA
...
!-----------------------------------------------------------------------
   ...
   attributes(device) SUBROUTINE hCapacity (patchtype,lb,nl_soil,csol,porsl,wice_soisno,wliq_soisno,scv,dz_soisno,cv)
...
! Snow heat capacity
   ...
   attributes(device) SUBROUTINE hConductivity (patchtype,lb,nl_soil,&
...
! - END -
   ...
   attributes(device) SUBROUTINE soil_hcap_cond(vf_gravels_s,vf_om_s,vf_sand_s,vf_pores_s,&
...
!      sr = max(1.0e-6, sr)
      ...
      IF(sr >= 1.0e-10) THEN
         select CASE (d_DEF_THERMAL_CONDUCTIVITY_SCHEME)
         CASE (1)
! -----------------------------------------------------------------------------------------
...
! -----------------------------------------------------------------------------------------
      ...
      IF (d_DEF_THERMAL_CONDUCTIVITY_SCHEME >= 1 .and. d_DEF_THERMAL_CONDUCTIVITY_SCHEME <=5) THEN
         ke = max(ke, 0.0)
         ke = min(ke, 1.0)
         IF(temperature > tfrz)THEN ! Unfrozen soil
      ...
      IF(d_DEF_THERMAL_CONDUCTIVITY_SCHEME == 6) THEN
! -----------------------------------------------------------------------------------------
...
! the degree of saturation of the minuscle pore space
      ...
      IF(d_DEF_THERMAL_CONDUCTIVITY_SCHEME == 7) THEN
! -----------------------------------------------------------------------------------------
...
! -----------------------------------------------------------------------------------------
      ...
      IF(d_DEF_THERMAL_CONDUCTIVITY_SCHEME == 8) THEN
! -----------------------------------------------------------------------------------------
...
END MODULE MOD_SoilThermalParameters_CUDA
! ---------- EOP ------------
